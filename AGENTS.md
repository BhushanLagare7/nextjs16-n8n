...
| **`inngest-realtime`**          | Streaming workflow updates to users — realtime channels, subscription tokens, React hooks, SSE consumers.          | Use v4 native realtime (`inngest/realtime` subpath). Define typed channels, publish from `step.run`, mint subscription tokens via server actions, consume in React with `useRealtime` from `inngest/react`. Do NOT install `@inngest/realtime` (v3-only package). |
